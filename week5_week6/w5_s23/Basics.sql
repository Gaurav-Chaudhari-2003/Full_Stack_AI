...
-- 1. SELECT Queries
-- Retrieve all columns or specific columns from the table.
...
-- 3. DISTINCT Clause
-- Retrieve unique values from a column to avoid duplicates.
SELECT DISTINCT location FROM emp.employee;
SELECT DISTINCT job_domain FROM emp.employee;

-- 4. LIKE Clause (Pattern Matching)
...
-- Filter records within a specific numeric or date range.
...
-- Date/Year Range (assuming integer year or date type)
...
-- 7. ORDER BY Clause
-- Sort records in ascending (default) or descending (DESC) order.
...
-- 9. OFFSET Clause
-- Skip a specific number of rows before returning results.
...
-- Sum (Common aggregate)
SELECT SUM(score) FROM employee WHERE job_domain = 'sales';

-- 12. Optimization
-- Combine multiple aggregate calculations into a single query for efficiency.
SELECT
